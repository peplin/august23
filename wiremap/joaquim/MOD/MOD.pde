...
// Modifications by Jukim: globe radius, bouncing added
...
// Variables for the Shape to be rendered (a globe)
  ...
  float radius = 200.00;                     // globe radius mod
...
int zdirection = 1;  // Left or Right
...
float zspeed = 1.5;  // Speed of the shape
...
void draw() {
  ...
  PmouseX = PmouseX + ( xspeed * xdirection );
  PmouseY = PmouseY + ( yspeed * ydirection );
  globe[1] = globe[1] + ( zspeed * zdirection );

  // Test to see if the shape exceeds the boundaries of the screen
  // If it does, reverse its direction by multiplying by -1
  if (PmouseX > width-radius || PmouseX-radius < 0) {
    xdirection *= -1;
  }
  if (PmouseY > height-radius || PmouseY-radius < 0) {
    ydirection *= -1;
  }

  if (globe[1] > height || globe[1] < 0) {
    zdirection *= -1;
  }


  globe[0] = (PmouseX / float(width)) * (map_length + radius * 2) - 
(map_length / 2 + radius);
  globe[2] = depth + radius - (PmouseY) / float(height) * (radius * 2 + 
960);
  fill(0);
  rect(0,0,width,height);                              // generate slivers only for hit wires
  for(int i=round((wire - wires_hit)/2); 
i<round((wire-wires_hit)/2)+wires_hit; i+=1){
    gen_circle(i);
...
{
  if((x_by_ind[i] >= (globe[0] - radius)) && (x_by_ind[i] <= (globe[0] + 
radius))) {          // if a wire's x is +/- radius inches of globe's
    float local_hyp = sqrt(sq(x_by_ind[i] - globe[0]) + sq(z_by_ind[i] - 
globe[2]));    // find the distance
    if(local_hyp <= radius) {                                                
               // if the distance from globe to wire's x/y is <radius
      float y_abs = sqrt(sq(radius) - sq(local_hyp));
      float y_top_coord = globe[1] + y_abs;
      float y_bot_coord = globe[1] - y_abs;                                  
            // calculate y's intersect

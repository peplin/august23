...
float radius = 200.0;                     // globe radius
...
// Variables for bouncing physics

int x_direction = 1;
int y_direction = -1;
int z_direction = 1;
//float gravity = -9800;
float gravity = 0 - 9800;
float coll_vel = 0;
float coll_x = 600;
float coll_t = 0;
int x_step = 6;
...
void draw() {
  ...
  // x = x + vt + 1/2 a t^2
  globe[1] = coll_x + coll_vel * (now - coll_t) + .5 * gravity * sq(now - coll_t);
  if (globe[1] < 0) {
    calc_coll();
    }
  fill(0);
...
void calc_coll() {
  ...
  float last_t = coll_t;
  float last_vel = coll_vel;
  float last_x = coll_x;

  println("last " + last_t + " " + last_vel + " " + last_x);
  
  if (last_x == 600) {
  coll_t = (-last_vel - sqrt(sq(last_vel) - 4 * .5 * gravity * last_x)) / (2 * .5 * gravity);
  coll_vel = - gravity * (coll_t - last_t) - last_vel;
  coll_x = last_x + last_vel * coll_t + .5 * gravity * sq(coll_t);
  } else {
  coll_t = last_t + abs((2 * last_vel)/(gravity));
  coll_vel =  last_vel;
  coll_x = 0;
  }

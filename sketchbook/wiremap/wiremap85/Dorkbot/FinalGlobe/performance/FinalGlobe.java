import processing.core.*; import java.applet.*; import java.awt.*; import java.awt.image.*; import java.awt.event.*; import java.io.*; import java.net.*; import java.text.*; import java.util.*; import java.util.zip.*; public class FinalGlobe extends PApplet {// Wiremap Renderer for GLOBE
// by
...
// Variables for the Projector
  ...
  float sliver_ratio = 0.85f;                 // ratio each sliver gets hit
...
// Varaibles for key input
  ...
  float step = .3f;                           // how far the globe moves / button press
  boolean mouse = true;
  int colorval_r = 0;
  int colorval_g = 0;
  int colorval_b = 255;
...
public void draw() {
  ...
  if (mouse==true){
    if (xpin == true ){
      globe[1] = ((height - mouseY) / PApplet.parseFloat(height)) * (15 + radius * 2) - (radius);
      globe[2] = depth + radius - (width - mouseX) / PApplet.parseFloat(width) * (16.0f + radius + 2);
    } else if (ypin == true){
      globe[0] = (mouseX / PApplet.parseFloat(width)) * (map_length + radius * 2) - (map_length / 2 + radius);
      globe[2] = depth + radius - (mouseY) / PApplet.parseFloat(height) * (16.0f + radius + 2);
    } else {
      globe[0] = (mouseX / PApplet.parseFloat(width)) * (map_length + radius * 2) - (map_length / 2 + radius);
      globe[1] = ((height - mouseY) / PApplet.parseFloat(height)) * (15 + radius * 2) - (radius);
    }      
...
public void gen_globe(float x, float y, float z, float rad){
...
public void gen_beat(){
  radius = 5 + 3 * cos(TWO_PI * beat_multiplier * PApplet.parseFloat((millis() - offset) % period) / PApplet.parseFloat(period));
}

public void gen_flash(){
  if( cos(TWO_PI * beat_multiplier * PApplet.parseFloat((millis() - offset) % period) / PApplet.parseFloat(period)) > 0 ){
    radius = 5;
  } else {
    radius = 0;
  }
...
public void gen_disco_ball() {
  int clicks = (floor(beat_multiplier * PApplet.parseFloat(millis() - offset) / PApplet.parseFloat(period))) % 8;
  switch(clicks){
    case 0:
      colorval_r = 0;
      colorval_g = 0;
...
static public void main(String args[]) {   PApplet.main(new String[] { "FinalGlobe" });}}
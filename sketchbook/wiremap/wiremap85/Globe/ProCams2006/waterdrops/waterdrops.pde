...
// Variables for Physical Wiremap
  ...
  float wire_height = 16.0;                  // height of wire in inches
...
// Variables for water drops
  float[] t_by_ind = new float[wire];        // time begin for each wire
  float time_interval = .51;                  // time interval between each drip
  float accel = .1;                       // for this program, i'm making acceleration 10 inches/second squared  
  int distinct_dots = int(ceil(sqrt((2*wire_height)/accel)/time_interval));  
...
map[84] = 3.0;
  ...
  for(int j=0; j<wire; j++) {
    t_by_ind[j] = random(time_interval);
  }
}
...
void gen_circle(int i)
{
  if((x_by_ind[i] >= (globe[0] - radius)) && (x_by_ind[i] <= (globe[0] + radius))) {          // if a wire's x is +/- radius inches of globe's
    ...
    if(local_hyp <= radius) {                                                              // if the distance from globe to wire's x/y is <radius
      ...
      float time_now = float(millis())/1000;
      
      for(int k=0; k<distinct_dots; k++) {

        float time_offset = time_now % t_by_ind[i];
        fill(0,0,255);
        stroke(0,0,255);
        float drop_top = accel * sq(time_offset *(k + 1)) / 2;
        if (drop_top < y_top_coord && drop_top > y_bot_coord) {
          line(i * 10, drop_top * ppi, i * 10 + 7, drop_top * ppi);
        }
      }

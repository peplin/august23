...
void setup() {
  ...
  lensImage = createGraphics(mysize1, mysize2, JAVA2D);
  lensImage2 = createGraphics(mysize1, mysize2, JAVA2D);
...
void draw() {
  ...
  int[] lensArray = new int[mysize1*mysize2];  // Height and width of lens
  // int[] lensArray = new int[width*height];
  ...
  for (int y = -r; y < r; y++) {
    for (int x = -r ;x < r; x++) {
      ...
      k1 = (y + r)*(lensD+check) + (x + r);
      k2 = (b+ r)*(lensD+check) + (a + r);
      //    if (k1 != k2 ) {println(k1+" "+k2+" "+r+" "+x+" "+ y);}
      if (k2 < 0) {
        println(k1+" "+k2);
      }
      lensArray[k1] = k2;
  ...
  lensImage = createGraphics(mysize1, mysize2, JAVA2D);
  //lensImage = createGraphics(width, height, JAVA2D);
  ...
  lensImage2.copy(lensEffect, 0, 0, mysize1, mysize2, 0, 0, mysize1, mysize2);
  //lensImage2.copy(lensEffect, 0,0, width, height, 0, 0, width, height);
  ...
  image(lensImage, 0, 0, mysize1, mysize2);
  //image(lensImage, xx, yy, width, height); 

...
void load() {  // cube position, cube dimensions, corner dimensions
  ...
  pitch(.1, cube_pos[1], cube_pos[2]);
  yaw(.1, cube_pos[0], cube_pos[2]);
  roll(.1, cube_pos[0], cube_pos[1]);
}


void pitch(float rad, float refy, float refz)
{
  for (int i = 0; i < 8; i ++ )
  {
    float z_dist = corner[i][2] - refz;
    float y_dist = corner[i][1] - refy;    
    float hyp = sqrt(sq(z_dist) + sq(y_dist));
    float theta = atan2(z_dist, y_dist);
    theta = (theta + (rad));
    corner[i][1] = refy + hyp * cos(theta);
    corner[i][2] = refz + hyp * sin(theta);
  }
    float z_dist = cube_pos[2] - refz;
    float y_dist = cube_pos[1] - refy;    
    float hyp = sqrt(sq(z_dist) + sq(y_dist));
    float theta = atan2(z_dist, y_dist);
    theta = (theta + (rad));
    cube_pos[1] = refy + hyp * cos(theta);
    cube_pos[2] = refz + hyp * sin(theta);

}

void yaw(float rad, float refx, float refz)
{
  for (int i = 0; i < 8; i ++ )
  {
    float x_dist = corner[i][0] - refx;
    float z_dist = corner[i][2] - refz;    
    float hyp = sqrt(sq(x_dist) + sq(z_dist));
    float theta = atan2(x_dist, z_dist);
    theta = (theta + (rad));
    corner[i][2] = refz + hyp * cos(theta);
    corner[i][0] = refx + hyp * sin(theta);
  }
    float x_dist = cube_pos[0] - refx;
    float z_dist = cube_pos[2] - refz;    
    float hyp = sqrt(sq(x_dist) + sq(z_dist));
    float theta = atan2(x_dist, z_dist);
    theta = (theta + (rad));
    cube_pos[2] = refz + hyp * cos(theta);
    cube_pos[0] = refx + hyp * sin(theta);

}

void roll(float rad, float refx, float refy)
{
  for (int i = 0; i < 8; i ++ )
  {
    float x_dist = corner[i][0] - refx;
    ...
    theta = (theta + (rad));
    corner[i][0] = refx + hyp * cos(theta);
    corner[i][1] = refy + hyp * sin(theta);
  }
    float x_dist = cube_pos[0] - refx;
    float y_dist = cube_pos[1] - refy;    
    float hyp = sqrt(sq(x_dist) + sq(y_dist));
    float theta = atan2(y_dist, x_dist);
    theta = (theta + (rad));
    cube_pos[0] = refx + hyp * cos(theta);
    cube_pos[1] = refy + hyp * sin(theta);
}
...
void keyPressed() {
  if (key == 'w') {
    pitch( PI / 32, corner[bot_corner][1], corner[bot_corner][2]);
  }
  else if (key == 's'){
    pitch( -PI / 32, corner[bot_corner][1], corner[bot_corner][2]);
  }
  else if (key == 'a'){
    yaw( -PI / 32, corner[bot_corner][0], corner[bot_corner][2] );
  }
  else if (key == 'd'){
    yaw( PI / 32, corner[bot_corner][0], corner[bot_corner][2] );
  }
  else if (key == 'q'){
    roll( PI / 32, corner[bot_corner][0], corner[bot_corner][1] );
  }
  else if (key == 'e'){
    roll( - PI / 32, corner[bot_corner][0], corner[bot_corner][1] );
  } 
  else if (key == 't') {
    pitch( PI / 32, corner[6][1], corner[6][2]);
  }
  else if (key == 'g'){
    pitch( -PI / 32, corner[6][1], corner[6][2]);
  }
  else if (key == 'f'){
    yaw( -PI / 32, corner[6][0], corner[6][2] );
  }
  else if (key == 'h'){
    yaw( PI / 32, corner[6][0], corner[6][2] );
  }
  else if (key == 'r'){
    roll( PI / 32, corner[6][0], corner[6][1] );
  }
  else if (key == 'y'){
    roll( - PI / 32, corner[6][0], corner[6][1] );
  } 
